const content = {
  pageType: "jh-page", pageId: "classListOfOneStudent", table: "class", pageName: '学生的班级列表', template: "jhTemplateV4", version: 'v2', 
  resourceList: [
    {
  ...
  headContent: [
    { tag: 'jh-page-title', value: "学生的班级列表", attrs: { cols: 12, sm: 6, md:4 }, helpBtn: true, slot: [], backBtn: {
      ...
      contentList: [
        {
          ...
          value: /*html*/`
            ...
            <v-data-table :headers="headers" :items="tableDataAllot" :search="searchInputDrawer"
              ...
              item-key="classId"
              checkbox-color="success" show-select v-model="allotTableSelected">
  ...
  common: { 
    ...
    doUiAction: {
      ...
      redirect: ['redirectToStudentList']
    }, // 额外uiAction { [key]: [action1, action2]}
    methods: {
      // ---------- 校验url传参 uiAction >>>>>>>>>> --------
      async prepareUrlObj() {
        const urlObj = new URLSearchParams(location.search);
        const studentId = urlObj.get('studentId');
      ...
      // ---------- 学生列表 uiAction >>>>>>>>>>>> --------
      async redirectToStudentList(funObj) {
        window.location.href = `/${window.appInfo.appId}/page/studentList`
      },
      // ---------- <<<<<<<<<<< 学生列表 uiAction  --------
    }
  },
